...
class DatabaseService {
    ...
    `
        ...
        try {
            ...
            // Add columns missing on pre-existing databases (migrations)
            await DatabaseService.addSecurityLevelColumn()
            await DatabaseService.addServiceOrderColumn()
        } catch (err) {
            console.error('Error creating the Service table:', err.message)
        }
    }

    // Add service_order column to existing databases (migration). Named
    // service_order because "order" is a reserved SQL keyword.
    static async addServiceOrderColumn() {
        try {
            const tableInfo = await DatabaseService.db.all(`PRAGMA table_info(Service)`)
            if (!tableInfo.some(col => col.name === 'service_order')) {
                await DatabaseService.db.exec(`ALTER TABLE Service ADD COLUMN service_order TEXT`)
                console.log('Added service_order column to Service table.')
            }
        } catch (err) {
            debug('Note: service_order column migration:', err.message)
        }
    }
    ...
    static async insertService(data) {
        try {
            ...
            const insertSQL = `
    INSERT INTO Service (name, serviceName, host, healthcheck, label, type, security_level, accoustic, model_quality, model_type, language, service_order)
    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    `
    ...
    static async getAllServices() {
        try {
            ...
            return services.map(service => {
                ...
                // Expose the stored column under the `order` name the rest of
                // the code expects (column is service_order: "order" is reserved).
